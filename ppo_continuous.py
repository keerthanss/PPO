...
class PPO:
    ...
    def loss(self, states: tf.Tensor, actions: tf.Tensor, action_probs: tf.Tensor, advantages: tf.Tensor, value_targets: tf.Tensor) -> tf.Tensor:
        ...
        mu, std = self.model.pi(states)
        dist = self.tfd.Normal(mu, std)
        respective_action_probs = dist.prob(actions)

        # calculate policy loss
        ratio = tf.math.exp(tf.math.log(tf.math.maximum(respective_action_probs,1e-10)) - tf.math.log(tf.math.maximum(action_probs,1e-10)))
        ...
        # the entropy of gaussian distribution is 0.5*log(2*pi*e*std^2) = 0.5*log(2*pi*std^2) + log(std)
        entropy = tf.math.log(std) + 0.5*np.log(2*np.pi*np.e)
