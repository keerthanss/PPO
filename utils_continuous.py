...
class ActorCritic(tf.keras.Model):
    ...
    def __init__(self, num_hidden_units):
        ...
        self.mu_layer = layers.Dense(1, name='mu', activation='tanh')
        self.std_layer = layers.Dense(1, name='std', activation='softplus')
...
class Trajectory:
    ...
    def _make_transition_tensors(self):
        ...
        self.actions_tensor = tf.constant(self.actions, dtype=tf.float32)
...
class EnvHelper:
    ...
    def run_episode(self, model:tf.keras.Model):
        ...
        for t in range(self.horizon):
            mu, std, value_t = model.predict(tf.constant(np.expand_dims(state,0), dtype=tf.float32))
            # print(mu, std)
            mu = np.squeeze(mu.numpy())
            std = np.squeeze(std.numpy())
            value_t = np.squeeze(value_t.numpy())
            action_t = np.random.normal(mu, std)
            action_prob = self.gaussian(mu, std, action_t)

            new_state, reward, done, _ = self.env.step(np.expand_dims(action_t,0))
            # self.env.render()
            # print(state, action_t, action_prob, reward, value_t)
            traj.add_transition(state, action_t, action_prob, reward/10.0, value_t)
            if done:
                break
            state = new_state
    ...
    def run_and_save_episode(self, model, video_path):
        ...
        while not done:
            ...
            mu, std, _ = model.predict(tf.constant(np.expand_dims(state,0), dtype=tf.float32))
            mu = np.squeeze(mu.numpy())
            std = np.squeeze(std.numpy())
            action_t = np.random.normal(mu, std)
            new_state, reward, done, _ = self.env.step(np.expand_dims(action_t,0))
